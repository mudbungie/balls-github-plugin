...
`balls-plugin-github-issues` is the GitHub-Issues mirror. Both
binaries share the `balls-github-shared` library crate (auth, HTTP,
config base, protocol types) and live in this repo, but they ship as
**separate participants** — separate names, separate projections,
separate auth-dirs. See the workspace README for the why. The
lifecycle contract this crate implements is specified in balls's
...
> Issue-tracker plugins (Jira, Linear, GitHub Issues — the sibling
> binary in this workspace) are a different family.
